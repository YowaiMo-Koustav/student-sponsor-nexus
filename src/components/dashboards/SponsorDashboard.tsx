...
import RecommendedEvents from '@/components/analytics/RecommendedEvents';
import SponsoredEventsPortfolio from '@/components/analytics/SponsoredEventsPortfolio';
import ROITrackingChart from '@/components/analytics/ROITrackingChart';
import StudentEngagementChart from '@/components/analytics/StudentEngagementChart';
import { 
  Users, 
  TrendingUp, 
  Target,
      ...
      {sponsorProfile && (
        <Tabs defaultValue="overview">
                ...
                {sponsorProfile ? (
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Badge variant="outline">{sponsorProfile.budget_range || 'Not specified' as React.ReactNode}</Badge>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {sponsorProfile.target_demographics.map((demo, index) => (
                            <Badge key={index} variant="secondary">{demo as React.ReactNode}</Badge>
                          ))}
                        </div>
                      </div>
                    )}
                  ...
                  {matches.slice(0, 3).map((match) => (
                    <div key={match.id} className="border rounded-lg p-4 hover:shadow-card transition-shadow">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <Badge className={getStatusColor(match.status)} variant="secondary">
                            {match.status as React.ReactNode}
                          </Badge>
                        </div>
                      </div>
                      <div className="flex items-center gap-4 text-sm text-muted-foreground mb-3">
                        ...
                        <Badge variant="outline">{match.events.category as React.ReactNode}</Badge>
                      </div>
          ...
          <TabsContent value="recommendations">
            <RecommendedEvents sponsorId={sponsorProfile.id} />
          </TabsContent>
          
          <TabsContent value="analytics">
            <SponsoredEventsPortfolio />
            <div className="mt-8">
                <ROITrackingChart />
            </div>
            <div className="mt-8">
          ...
          <TabsContent value="profile">
            <CompanyProfileForm profile={sponsorProfile} onProfileUpdate={fetchSponsorProfile} />
          </TabsContent>
        </Tabs>
      )}
    </div>
